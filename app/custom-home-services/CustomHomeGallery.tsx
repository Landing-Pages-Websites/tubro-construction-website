...
export function CustomHomeGallery({ section, images }: { section: ManifestSection; images: SectionImage[] }): ReactElement {
  return (
    <section id={section.name} className={styles.gallery} aria-labelledby="gallery-heading">
      <div className={styles.galleryIntro}><p className={styles.eyebrow}>Materials. Spaces. Possibilities.</p><h2 id="gallery-heading">{section.content.headline}</h2><p className={styles.body}>Explore exterior finishes, timber details, and outdoor spaces from Tubro’s residential project portfolio.</p><a className={styles.textLink} href="/recent-projects">{section.content.cta}<ArrowUpRight aria-hidden="true" /></a></div>
      <figure className={styles.materialPhoto}><div><Image src={images[2].src} alt={images[2].alt} fill sizes="(min-width: 761px) 37vw, 100vw" /></div><figcaption><span>{images[2].label}</span><span>Care in the details</span></figcaption></figure>
      <figure className={styles.deckPhoto}><div><Image src={images[1].src} alt={images[1].alt} fill sizes="(min-width: 761px) 55vw, 100vw" /></div><figcaption><span>{images[1].label}</span><span>Space to live outside</span></figcaption></figure>
