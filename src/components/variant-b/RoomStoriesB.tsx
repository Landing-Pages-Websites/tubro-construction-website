  ...
  return (
    <section id="rooms" aria-labelledby="rooms-b-heading" className={styles.rooms}>
      ...
      <div className={styles.roomPhotos}>
        {ROOM_PHOTOS.map(({ image, copy }) => <figure key={copy.title} className={styles.roomFigure}><div className={styles.roomImage}><Image src={image.src} alt={image.alt} fill sizes="(min-width: 1024px) 54vw, 100vw" className={styles.image} /></div><figcaption><span>{copy.title}</span><p>{copy.body}</p></figcaption></figure>)}
      </div>
